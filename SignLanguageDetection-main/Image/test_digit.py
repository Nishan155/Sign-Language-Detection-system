...
class DigitRecognizer:
    ...
    def load_model(self):
        info_path = os.path.join(self.model_dir, "best_digits_model_info.pkl")
        if not os.path.exists(info_path):
            raise FileNotFoundError("Train a digit model first.")

        with open(info_path, "rb") as f:
            info = pickle.load(f)
        model_name = info["best_model_name"]
        model_path = os.path.join(self.model_dir, f"{model_name}_digits_model.pkl")
...
def capture_image():
    ...
    while True:
        ...
        if key == 27:
            ...
            filename = f"captured_digits/{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
            os.makedirs("captured_digits", exist_ok=True)
...
def main():
    ...
    digit, confidence = recognizer.predict(image)
    if digit:
